...
class PSFprediction_guided_CACModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual:
            # 兼容两种接口：tuple (l_percep, l_style) 或 单 tensor（如 myLoss256 未改返回值时）
            out = self.cri_perceptual(self.output, self.gt)
            if isinstance(out, (tuple, list)) and len(out) == 2:
                l_percep, l_style = out
            else:
                l_percep, l_style = out, None
            if l_percep is not None:
                l_g_total += l_percep.mean() if l_percep.dim() > 0 else l_percep
                loss_dict['l_percep'] = l_percep.mean() if l_percep.dim() > 0 else l_percep
